...
for seed_val in seed_vals:  # loop across seed vals
    ...
    for encoder_arch in encoder_archs:  # loop across encoder archs
        ...
        for classifier_arch in class_archs:
            subplot_idx_enc = 0
            enc_str = 'Linear' if encoder_arch == [] else 'Nonlinear'
            class_arch_str = 'Linear' if classifier_arch == [] else 'Nonlinear'
            row_label = f'{enc_str} Encoder {class_arch_str} Classifier'
            axs[subplot_idx_class][0].set_ylabel(row_label, rotation=0, labelpad=100)
